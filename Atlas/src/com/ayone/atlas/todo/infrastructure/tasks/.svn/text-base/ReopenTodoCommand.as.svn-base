...
[Bindable]
public class ReopenTodoCommand implements ICommand, IChangeTodoStatusCommand
{
	//------------------------------------------------------------
	//
	// Properties
	...
	{	
		...
		service.reopenTodo.addEventListener("result", resolveTodoResultHandler);
		service.addEventListener("fault", faultHandler);
		service.reopenTodo(todoItem.id);
	}
